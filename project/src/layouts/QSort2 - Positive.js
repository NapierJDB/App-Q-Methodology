...
export default class QSort2 extends Component {
    ...
    getArrays() {
        ...
        var asString = localStorage.getItem('GREEN_BOX');
        this.state.positiveAnchors = localStorage.getItem('RE_POSITIVE_ANCHORS');
    ...
    getVariable(){
        ...
        if(this.state.index == 0){ 
            ...
            this.state.statement = this.state.greenBoxStatements[this.state.index];
        }
        else
        {           
            this.state.statement = this.state.greenBoxStatements[this.state.index];
    ...
    addStatement() {
        ...
        if(this.state.quantity[anchorsIndex] > 0){
            ...
            let remove = this.state.greenBoxStatements.indexOf(this.state.greenBoxStatements[this.state.index]);
            this.setState({
                greenBoxStatements: this.state.greenBoxStatements.filter((_, i) => i !== remove)
            })
        }
        else{
    ...
    manageNavigation(){
        ...
        if(this.state.greenBoxStatements.length == 0){
    ...
    sendResultsToLocalStorage(){
        localStorage.setItem('POSITIVE_RESULTS', JSON.stringify(this.state.list));
    }
    ...
    render() {
        ...
        const mappedList = this.state.list.map((item, index) => {
         ...
         let {index, greenBoxStatements} = this.state;
